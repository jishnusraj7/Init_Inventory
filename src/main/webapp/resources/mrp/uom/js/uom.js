...
function uomctrl($controller,$compile,$timeout,$scope, $http, $mdDialog ,$rootScope, DTOptionsBuilder,DTColumnBuilder,
...
/*$("#form_div_description").hide();*/
      ...
      /*  set_sub_menu("#settings");						
        setMenuSelected("#uom_left_menu");	*/		//active leftmenu
        manageButtons("add");
...
//    	generate number
	...
	function edit(row_data,cur_row_index,event) {									
		...
		$scope.formData = {id:response.data.editDet.id,name:response.data.editDet.name,code:response.data.editDet.code,
				...
				base_uom_id:response.data.editDet.base_uom_id,compound_unit:response.data.editDet.compound_unit,
				uom_symbol:response.data.editDet.uom_symbol,decimal_places:response.data.editDet.decimal_places};		
		$scope.createdBy = response.data.editDet.created_by;
		$scope.createdAt =  response.data.editDet.created_at;
		if(response.data.editDet.is_compound==1)
			{
			$scope.formData.is_compound=true;
			}
		else {
			$scope.formData.is_compound=false;
			}
...
$rootScope.$on("fun_next_rowData",function(e,id){
	...
	function code_existing_validation(data){
		...
		if($scope.formData.is_compound==true)
			{
			if($scope.formData.base_uom_id=="")
				{
				$("#form_div_base_uom_id").addClass("has-error");
				$("#form_div_base_uom_id_error").show();
				flg = false;
				}
			else
				{
			...
			if($scope.formData.compound_unit=="")
			{
				$("#form_div_compound_unit").addClass("has-error");
				$("#form_div_compound_unit_error").show();
				flg = false;
				}
			else
				{
