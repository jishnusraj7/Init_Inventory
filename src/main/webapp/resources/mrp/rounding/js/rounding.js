...
mrpApp.controller('roundingctrl', roundingctrl);

function roundingctrl($controller,$compile,$timeout,$scope, $http, $mdDialog ,$rootScope, DTOptionsBuilder,DTColumnBuilder,
...
/*$("#form_div_description").hide();*/
      ...
      /*  set_sub_menu("#settings");						
        setMenuSelected("#rounding_left_menu");	*/		//active leftmenu
        manageButtons("add");
...
//    	generate number
	    ...
	    vm.dtColumns = [
			        ...
			        DTColumnBuilder.newColumn('round_to').withTitle('ROUNDING TO').withOption('width','400px'),
	...
	function edit(row_data,cur_row_index,event) {									
		...
		$scope.formData = {id:response.data.editDet.id,name:response.data.editDet.name,code:response.data.editDet.code,
				...
				round_to:response.data.editDet.round_to};		
		...
		/*$("#form_div_rounding_to").removeClass("has-error");
		$("#form_div_rounding_to_error").hide();*/
	}
