...
mrpApp.controller('department', department);


function department($controller,$compile,$timeout,$scope, $http, $mdDialog ,$rootScope, DTOptionsBuilder,DTColumnBuilder,$q,
...
/*	set_sub_menu("#settings");		
	...
	vm.dtColumns = [
	                DTColumnBuilder.newColumn('code').withTitle('CODE').withOption('type', 'natural').withOption('width','250px').renderWith(
	                		...
	                		DTColumnBuilder.newColumn('dept_type_name').withTitle('DEPARTMENT TYPE').withOption('width','250px'),
...
//	Delete Function
	...
	function code_existing_validation(data){
		...
		if(validation() == false){
			flg = false;	}
		if($scope.formData.dept_type == undefined){
			$("#form_div_dept_type").addClass("has-error");
			$("#form_div_dept_type_error").show();
