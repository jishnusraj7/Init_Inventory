...
mrpApp.controller('reasons', reasons);


function reasons($controller,$compile,$timeout,$scope, $http, $mdDialog ,$rootScope, DTOptionsBuilder,DTColumnBuilder,
...
//	generate number
	...
	setMenuSelected("#reasons_leftmenu");			//active leftmenu
	manageButtons("add");

	$scope.formData = {context:""};
	...
	vm.dtColumns = [
	                DTColumnBuilder.newColumn('code').withTitle('CODE').withOption('type', 'natural').withOption('width','250px').renderWith(
	                		...
	                		DTColumnBuilder.newColumn('context_type').withTitle('CONTEXT').withOption('width','250px'),
	...
	function edit(row_data,cur_row_index,event) {									
		...
		$scope.formData = {id:row_data.id,name:row_data.name,code:row_data.code,description:row_data.description,context:row_data.context};
...
//	Delete Function
	...
	function code_existing_validation(data){
		...
		if(validation() == false){
			flg = false;	}
		if($scope.formData.context == undefined || $scope.formData.context =="" ){
			$("#form_div_context").addClass("has-error");
			$("#form_div_context_error").show();

			flg = false;
		}else{
			$("#form_div_context").removeClass("has-error");
			$("#form_div_context_error").hide();
		}
	...
	$scope.filterContext=function()
	{
		//alert($scope.formData.context);
	}
