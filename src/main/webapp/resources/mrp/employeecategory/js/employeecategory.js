...
/*	set_sub_menu("#settings");		
	setMenuSelected("#employeecategory_left_menu");	*/		//active leftmenu
	manageButtons("add");
	...
	$rootScope.$on("fun_delete_current_data",function(event){			
		...
		$mdDialog.show(confirm).then(function() {
			...
			$http({
				...
				if(response.data==0)
				{
					$rootScope.$broadcast('on_AlertMessage_ERR',"Employee Category "+FORM_MESSAGES.ALREADY_USE+"");
				}
