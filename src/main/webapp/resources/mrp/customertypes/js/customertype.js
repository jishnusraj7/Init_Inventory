...
mrpApp.controller('customertype', customertype);


function customertype($controller,$compile,$timeout,$scope, $http, $mdDialog ,$rootScope, DTOptionsBuilder,DTColumnBuilder,
...
//	generate number
	...
	setMenuSelected("#customertypes_leftmenu");	*/		//active leftmenu
	manageButtons("add");
	...
	vm.dtColumns = [
	                DTColumnBuilder.newColumn('code').withTitle('CODE').withOption('type', 'natural').withOption('width','200px').renderWith(
	                		...
	                		DTColumnBuilder.newColumn('default_price_variance_pc').withTitle('PRICE VARIANCE(%)').withClass("dt-body-right").withOption('width','200px').renderWith(
	                				function(data, type, full, meta) {
	...
	function edit(row_data,cur_row_index,event) {									
		...
		$scope.formData = {id:row_data.id,name:row_data.name,code:row_data.code,description:row_data.description,
				price_var:row_data.default_price_variance_pc,created_at:row_data.created_at,created_by:row_data.created_by};
...
//	Delete Function
	$rootScope.$on("fun_delete_current_data",function(event){			
		...
		$mdDialog.show(confirm).then(function() {
			...
			$http({
				...
				if(response.data==0)
				{
					$rootScope.$broadcast('on_AlertMessage_ERR',"Customer Type "+FORM_MESSAGES.ALREADY_USE+"");
				}
	...
	$rootScope.$on('fun_save_data',function(event){		//Save Function
		if($scope.formData.price_var !="" && $scope.formData.price_var !=null && $scope.formData.price_var !=undefined)
		{
		  $scope.formData.default_price_variance_pc=parseFloat($scope.formData.price_var);
	...
	function code_existing_validation(data){
		...
		if(validation() == false){
			flg = false;	}
		else if($scope.formData.default_price_variance_pc =="" || $scope.formData.default_price_variance_pc == undefined ||$scope.formData.default_price_variance_pc == "")
		{
		 
			 flg = false;
			 $("#form_div_price_var").addClass("has-error");
			 $("#form_div_price_error").show();
			
		}else if($scope.formData.default_price_variance_pc>99.99)
			{
			flg=false;
			$("#divErrormsg2").text('Please Enter A Valid Price variance!');
			$("#divErrormsg2").show();
			 $("#form_div_price_var").addClass("has-error");
			
			}else
				{
				 $("#form_div_price_var").removeClass("has-error");
				 $("#form_div_price_error").hide();
				 $("#divErrormsg2").hide();
				}
