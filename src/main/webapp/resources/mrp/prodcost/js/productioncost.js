...
mrpApp.controller('productioncost', productioncost);


function productioncost($controller,$compile,$timeout,$scope, $http, $mdDialog ,$rootScope, DTOptionsBuilder,DTColumnBuilder,
...
/*	set_sub_menu("#settings");		
	setMenuSelected("#department_left_menu");*/			//active leftmenu
	manageButtons("add");
	...
	vm.dtColumns = [
	                DTColumnBuilder.newColumn('code').withTitle('CODE').withOption('type', 'natural').withOption('width','250px').renderWith(
	                		...
	                		DTColumnBuilder.newColumn('cost_type_name').withTitle('COST TYPE').withOption('width','250px'),
	...
	function edit(row_data,cur_row_index,event) {									
		...
		$scope.formData = {id:row_data.id,name:row_data.name,code:row_data.code,description:row_data.description,cost_type:row_data.cost_type};
...
//	Delete Function
	$rootScope.$on("fun_delete_current_data",function(event){			
		...
		$mdDialog.show(confirm).then(function() {
			...
			$http({
				...
				if(response.data==0)
				{
					$rootScope.$broadcast('on_AlertMessage_ERR',"Production Cost "+FORM_MESSAGES.ALREADY_USE+"");
				}
	...
	{
		window.open("ProductionCostReport.xls");
	}
	...
	function code_existing_validation(data){
		...
		if(validation() == false){
			flg = false;	}
		if($scope.formData.cost_type == undefined){
			$("#form_div_cost_type").addClass("has-error");
			$("#form_div_cost_type_error").show();

			flg = false;
		}else{
			$("#form_div_cost_type").removeClass("has-error");
			$("#form_div_cost_type_error").hide();
		}
