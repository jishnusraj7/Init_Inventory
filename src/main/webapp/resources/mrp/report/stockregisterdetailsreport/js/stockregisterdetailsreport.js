var myApp = angular.module('stock-register_details_report_app', [ 'ngMaterial',
		'ngMessages', 'common_app','checklist-model' ]);

myApp.controller('stockregisterDetailsCtrlr', stockregisterDetailsCtrlr);

function stockregisterDetailsCtrlr($scope, $http, $mdDialog, $rootScope,
		...
		if($scope.serch!="")
		  {
		    ...
		    $.each($scope.itemData , function (i, v) {
		...
		$scope.selectedItemList=[];
		 ...
		 $scope.btnRight = function () {
			...
			if($scope.user.roles.length!=0)
			  ...
			  for(var i=0;i<$scope.user.roles.length;i++)
				  {
				  for(var j=0;j<$scope.itemData.length;j++)
					  {
				 if( $scope.user.roles[i].id==$scope.itemData[j].id)
					 {
					 $scope.itemData.splice(j, 1);
					 }
				  }
				  }
		 ...
		 $scope.btnLeft = function () {
			 ...
			 for(var i=0;i<$scope.userSelect.roles.length;i++)
			  {
			  $scope.itemData.push($scope.userSelect.roles[i]);
			  }
			 //remove the ones that were moved from the source container.
						...
						if ($("#select_report").val() == "number:8") {
							...
							if ($("#optnDate").prop('checked') == true) {
								...
								if ($scope.formData.startdate != undefined
									...
									$window
											.open(
													"../stockregisterdetailsreport/Stock Register Detail?&option="
															+ option
															+ "&startdate="
															+ startdate
															+ "&enddate="
		...
		if ($("#select_report").val() == "number:8") {
			...
			if ($("#optnDate").prop('checked') == true) {
				...
				if ($scope.formData.startdate != undefined && $scope.formData.startdate != "" && $scope.formData.enddate != ""
					...
					$("#form_div_startmonth_error").hide();
					var link=document.createElement('a');
					link.href="../stockregisterdetailsreport/Stock Register Detail Excel?&option="+ option+ "&startdate="+ startdate
	...
	$scope.itemData = [];
	 ...
	 $http({
		 url : '../itemmaster/json',
			...
			$scope.itemData = itemresponse.data.data;
...
angular.bootstrap(document.getElementById("app_stock-register_details"),
		[ 'stock-register_details_report_app' ]);