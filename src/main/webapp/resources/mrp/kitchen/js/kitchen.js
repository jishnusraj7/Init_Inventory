...
mrpApp.controller('kitchen', kitchen);


function kitchen($controller,$compile,$timeout,$scope, $http, $mdDialog ,$rootScope, DTOptionsBuilder,DTColumnBuilder,
...
/*	set_sub_menu("#settings");		
	setMenuSelected("#kitchen_left_menu");	*/		//active leftmenu
	manageButtons("add");
	...
	vm.dtColumns = [
	                ...
	                DTColumnBuilder.newColumn('code').withTitle('CODE').withOption('type', 'natural').withOption('width','250px').renderWith(
	                		...
	                		DTColumnBuilder.newColumn('created_by').withTitle('CREATED BY').withOption('width','250px'),
	                		DTColumnBuilder.newColumn('created_at').withTitle('CREATED AT').notVisible().withOption('searchable', false),
	...
	function edit(row_data,cur_row_index,event) {									
		...
		$scope.formData = {id:row_data.id,name:row_data.name,code:row_data.code,description:row_data.description,dept_type:row_data.dept_type,created_at:row_data.created_at};
	...
	$rootScope.$on("fun_delete_current_data",function(event){			
		...
		$mdDialog.show(confirm).then(function() {
			...
			$http({
				...
				if(response.data==0)
				{
					$rootScope.$broadcast('on_AlertMessage_ERR',"Kitchen "+FORM_MESSAGES.ALREADY_USE+"");
				}
