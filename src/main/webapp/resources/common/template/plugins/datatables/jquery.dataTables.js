...
(/** @lends <global> */function( $ ) {
	...
	function _fnPageChange ( settings, action, redraw )
	{
		...
		else if ( action == "last" )
		{
			...
			var pageNo=parseInt($('#txtPageNum').val());
			if(isNaN(pageNo)){
			pageNo=1;
			}
			if(((pageNo-1)* len)>=records){
			start = Math.floor( (records-1) / len) * len;
			}else{			
			start=(pageNo-1)* len;
			}		
	...
	DataTable.defaults = {
		...
		"oLanguage": {
			...
			"oPaginate": {
				...
				/**
				 * Text to use for the 'search page number' pagination button (to take the user to
				 * the serched page).
				 *  @type string
				 *  @default go_to_page
				 *
	...
	$.extend( extPagination, {
		...
		page_search: function ( page, pages ) {
			return [ 'first', 'previous', 'search_page', 'next', 'last' ];
		},
					...
					for ( i=0, ien=buttons.length ; i<ien ; i++ ) {
						...
						if ( $.isArray( button ) ) {
							...
							if ( btnDisplay !== null ) {
								...
								if(btnDisplay==lang.sGotoPage){
									node = $('<input type="text" value="'+(page+1)+'" id="txtPageNum"/><a class="'+classes.sPageButton+' '+btnClass+'">', {
								...
								$('#txtPageNum').click(function() { return false; });
								$('#txtPageNum').off('click');
								counter++;
							}
						}
					}
