(function($) {
	...
	$.fn.tautocomplete = function(options, callback) {
		...
		var el = {
			...
			ddTextbox : $("<input type='text' maxlength='1000'>")
		};
		...
		this.wrap("<div class='acontainer search-div-width'></div>");
		...
		el.ddTextbox.keyup(function(e) {
			if (!(e.shiftKey && e.keyCode == 9)) {
				...
				if ((e.keyCode < 46 || e.keyCode > 105)
						&& (e.keyCode != keys.BACKSPACE) && (e.keyCode != 9)) {
					e.preventDefault();
					return;
				}
		...
		function ajaxData(jsonData) {
			if (settings.ajax.success == null || settings.ajax.success == ""
					|| (typeof settings.ajax.success === "undefined")) {
				jsonParser(jsonData);
			} else {
		...
		/*
		 * el.ddTextbox.keypress(function (event) { var regex = new
		 * RegExp(settings.regex); var key = String.fromCharCode(!event.charCode ?
		...
		el.ddTextbox.keydown(function(e) {
			...
			if (e.keyCode == keys.UP) {
				...
				if (selected.prev().length == 0) {
					$(".adropdown").scrollTop(300);

					tbody.find("tr:last").addClass("selected");
				} else {

					if (selected.index() == 7) {
						$(".adropdown").scrollTop(0);
		...
		function select() {
			...
			for ( var i = 0; i < cols; i++) {
				selectedData[settings.columns[i]] = selected.find('td').eq(i)
						.text();
			}

			el.ddTextbox.val(selected.find('td').eq(1).text());
			orginalTextBox.val(selected.find('td').eq(0).text() + '#$#'
					+ selected.find('td').eq(1).text());
			hideDropDown();
			onChange();
			el.ddTextbox.focus();
		}
		...
		function showDropDown() {
			...
			var cssBottom = "1px 1px " + (el.ddTextbox.height() + 20)
					+ "px 1px";
			...
			if (!isDivWidthVisible(el.ddDiv)) {
				el.ddDiv.css("left", "-"
						+ (el.ddTable.width() - el.ddTextbox.width() - 20)
						+ "px");
			}
		}
					...
					for (i = 0; i < jsonData.length; i++) {
						...
						for ( var key in obj) {
							...
							if (j <= cols) {
								...
								if (highlight) {
									cell = cell.replace(re, "<span class='"
											+ settings.highlight
											+ "'>$&</span>");
								}
								row = row + "<td>" + cell + "</td>";
