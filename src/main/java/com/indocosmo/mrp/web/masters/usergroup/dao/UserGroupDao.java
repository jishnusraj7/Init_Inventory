 ...
 *
 */
public class UserGroupDao extends MasterBaseDao<UserGroup> implements IUserGroupDao{

	public UserGroupDao(ApplicationContext context) {
		super(context);
	}
	 ...
	 */
	@Override
	public UserGroup getNewModelInstance() {
	...
	public JsonArray getTableRowsAsJsonForDataTable(String columns,String wherePart,String searchCriteria,String sortPart,int limitRows,int offset,String adnlFilterPart,List<DataTableColumns> columnList) throws Exception{
		...
		String sql = "SELECT itm.id,code,name,description,created_at,created_by,IFNULL(sync_queue.id ,'') AS quetableId "
				+ "FROM "+getTable()+" itm LEFT JOIN sync_queue  ON (itm.id=sync_queue.record_id AND sync_queue.table_name='"+getTable()+"') "
				+ ""+wherePart+" "+sortPart+" limit "+limitRows+" OFFSET "+offset+"";
	...
	public JsonArray getTableRowsAsJson() throws Exception {
		...
		final String sql="SELECT * FROM " + getTable()+" where is_deleted=0 OR is_deleted = NULL";
	...
	public Integer deleteSyncQue(Integer id) throws Exception {
		...
		st = con.prepareCall("{call usergrp_syn_delete(?)}");
