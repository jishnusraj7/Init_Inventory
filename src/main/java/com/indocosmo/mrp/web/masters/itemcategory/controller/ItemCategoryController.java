package com.indocosmo.mrp.web.masters.itemcategory.controller;
...
public class ItemCategoryController extends MasterBaseController<ItemCategory, ItemCategoryDao, ItemCategoryService> {
	...
	@Override
	public ItemCategoryService getService() {
	
		return new ItemCategoryService(getCurrentContext());
	}
	...
	public void getCounterWothPrefix(HttpServletResponse response , Model model) throws Exception {
		...
		{
		 CounterWithPrefix = counterService.getNextCounterwithPrefix("item_category", "item_category");
		}
	...
	public String getList(@ModelAttribute ItemCategory itemCategory , HttpSession session , Model model)
			throws Exception {
	
		final CurrentUserPermissionService userPermissionService = new CurrentUserPermissionService();
		
		SysdefPermission permission = userPermissionService.getCurrentUserPermission(session, "SET_ITM_CAT");
		
		model.addAttribute("permission", permission);
		model.addAttribute("Settings",true);
		model.addAttribute("Products",true);
		
		model.addAttribute("itemcategoryclass",true);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/saveItmCat", method = RequestMethod.POST)
	public void saveItmCat(@RequestBody String Quetable , HttpSession session , HttpServletResponse response ,
			HttpServletRequest request) throws Exception {
	
		final ItemCategoryService itemCategoryService = new ItemCategoryService(getCurrentContext());
		final ItemCategoryDao itemCategoryDao = new ItemCategoryDao(getCurrentContext());
		final SyncQueueService SyncQueueService = new SyncQueueService(getCurrentContext());
		
		JsonParser parser1 = new JsonParser();
		
		final String DB_TXN_POINT = "itemCategoryService";
		
		Gson gson = new Gson();
		ItemCategory itm = null;
		SyncQueue syncQueue = null;
		itemCategoryService.beginTrans(DB_TXN_POINT);
		...
		try {
			...
			if (itm.getId().toString() != "") {
				...
				String tableName = itemCategoryDao.getTable();
			...
			itemCategoryService.endTrans(DB_TXN_POINT);
		}
		catch (Exception e) {
			
			response.getWriter().print("0");
			itemCategoryService.rollbackTrans(DB_TXN_POINT);
			e.printStackTrace();
	...
	public void delete(HttpServletRequest request , HttpServletResponse response) throws Exception {
		...
		try {
			...
			Integer is_sync = 0;
			is_deleted = itemCategoryService.delete(id);
			if (is_deleted == 0) {
				...
				{
				is_sync = SyncQueueService.updateSyncQueue(id, itemCategoryDao.getTable());
				}
				response.getWriter().print("1");
			}
			itemCategoryService.endTrans(DB_TXN_POINT);
