package com.indocosmo.mrp.web.masters.combocontents.controller;
...
@Controller
@RequestMapping("/combocontents")
public class ComboContentsController extends MasterBaseController<ComboContents, ComboContentsDao, ComboContentsService> {
	
	/* (non-Javadoc)
	 * @see
	 * com.indocosmo.mrp.web.core.base.controller.BaseController#getService() */
	@Override
	public ComboContentsService getService() {
	
		return new ComboContentsService(getCurrentContext());
	}
	...
	public void saveSuppier(@RequestBody String Quetable ,  HttpSession session , HttpServletResponse response ,
		...
		final ComboContentsSubstituionService combocontentService = new ComboContentsSubstituionService(getCurrentContext());
		final ComboContentsSubstituionDao combocontentSubstitutionDao = new ComboContentsSubstituionDao(getCurrentContext());
		final ComboContentsDao combocontentDao = new ComboContentsDao(getCurrentContext());
		...
		Gson gson = new Gson();
		ComboContents combocontents = null;
		...
		try {
			...
			if (combocontents.getId().toString() != "") {
				...
				String tableName = combocontentDao.getTable();
			...
			String Combocontentdtl =jsonObject.get("pricesettings").getAsString();
			JsonParser parser = new JsonParser();
			JsonArray CombocontentdtlList = (JsonArray) parser.parse(Combocontentdtl);

		
			
			final ArrayList<ComboContentsSubstituion> combocontentList = new ArrayList<ComboContentsSubstituion>();
			...
			for (int i = 0; i < CombocontentdtlList.size(); i++) {
				...
				final ComboContentsSubstituion  combocontentsubstitution = new ComboContentsSubstituion();

				combocontentsubstitution.setIs_default(json.get("is_default").getAsInt());
				combocontentsubstitution.setPrice_diff(json.get("price_diff").getAsDouble());
				combocontentsubstitution.setQty(json.get("qty").getAsDouble());
				combocontentsubstitution.setSubstitution_sale_item_id(json.get("substitution_sale_item_id").getAsInt());
				combocontentsubstitution.setCombo_content_id(combocontents.getId());
